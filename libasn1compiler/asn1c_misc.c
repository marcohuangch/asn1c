...
asn1c_type_name(arg_t *arg, asn1p_expr_t *expr, enum tnfmt _format) {
	...
	switch(_format) {
	case TNF_UNMODIFIED:
		return asn1c_make_identifier(stdname ? AMI_MASK_ONLY_SPACES : AMI_MASK_ONLY_SPACES | AMI_USE_PREFIX,
	...
	case TNF_SAFE:
		return asn1c_make_identifier(stdname ? 0 : AMI_USE_PREFIX, exprid, typename, 0);
	...
	case TNF_RSAFE:	/* Recursion-safe type */
		return asn1c_make_identifier(stdname ? AMI_CHECK_RESERVED : AMI_CHECK_RESERVED | AMI_USE_PREFIX, 0,
			"struct", " ", typename, 0);
	}
